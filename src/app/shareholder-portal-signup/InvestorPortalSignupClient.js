...
export default function InvestorPortalSignupClient({ title, content, investorPortal }) {
                            ...
                            if (itemType === 'folder') {
                                ...
                                return (
                                    <div 
                                        ...
                                        className={`bg-gray-200 hover:bg-gray-200 transition-colors duration-200 cursor-pointer rounded-lg p-4 mb-2 flex items-center justify-between ${level > 0 ? 'ml-12' : 'ml-6'}`}
                                    >
                                        <div className="flex items-center gap-3">
                                            <Document />
    ...
    if (!user) {
        return <SignupForm title={title} content={content} />;
    }

    return (
        <>
            <Container className="pt-40 pb-10 2xl:pt-50 relative z-10 flex flex-col gap-10">
