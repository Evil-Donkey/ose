...
const Button = React.forwardRef(({ children, href, className, ...props }, ref) => {
    return href ? (
        <Link href={href} className={`bg-lightblue text-white text-sm lg:text-xs xl:text-base font-normal px-6 py-3 rounded-full shadow hover:bg-darkblue text-center transition-colors cursor-pointer w-max uppercase ${className}`} {...props}>{children}</Link>
    ) : (
        <button 
            ref={ref} 
