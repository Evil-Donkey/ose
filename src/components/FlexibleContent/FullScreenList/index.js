...
const FullScreenList = ({ data }) => {
    ...
    return (
        <div id={sectionLabel ? formatSectionLabel(sectionLabel) : undefined} className={`relative min-h-[100vh] ${list ? '2xl:min-h-auto' : ''} h-full w-full overflow-hidden flex flex-col ${list ? 'justify-end md:justify-center' : 'justify-start'}`}>
            <div
                ref={el => backgroundImageRef.current[0] = el}
                className={`absolute top-0 left-0 w-full h-full bg-cover bg-center scale-180 origin-top ${backgroundImageMobile?.mediaItemUrl ? 'hidden lg:block' : ''}`}
            ...
            {backgroundImageMobile?.mediaItemUrl && (
                <div
                    ref={el => backgroundImageRef.current[1] = el}
                    className="absolute top-0 left-0 w-full h-full bg-cover bg-center scale-180 origin-top lg:hidden"
                    style={{ backgroundImage: `url(${proxyImageUrl(backgroundImageMobile.mediaItemUrl)})` }}
                />
            )}
